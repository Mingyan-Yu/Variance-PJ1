...
## extract subject ID in group 1 and group 4
comparison_group_ID <- unique(survey$idno[survey$group %in% c(3,4)])
...
for(i in 1:HR.data$N){
  ...
  fitted_values_34 <- append(fitted_values_34, list(mu_i))
  f_values_34 <- append(f_values_34, list(fit_i))
}
...
for(i in 1:HR.data$N){
  ...
  for(j in 1:500){
    ...
    e_i <- rep(0, HR.data$n[i])
    e_i[1] <- AR1[j,i]*e_i0 + w[1]
    rep_i[j,1] <- f_values_34[[i]][j,1] + e_i[1]
    for(s in 2:HR.data$n[i]){
      e_i[s] <- AR1[j,i]*e_i[s-1] + w[s]
      rep_i[j,s] <- f_values_34[[i]][j,s] + e_i[s]
    }
  }
  rep_values_34 <- append(rep_values_34, list(rep_i))
}
...
for(i in 1:HR.data$N){
  T_obs_i <- rep(0, 500)
  for(j in 1:500){
    T_obs_i[j] <- sum((HR.data$HR[i,1:HR.data$n[i]]-fitted_values_34[[i]][j,1:HR.data$n[i]])^2/
  ...
  T_obs_34 <- append(T_obs_34, list(T_obs_i))
}
...
for(i in 1:HR.data$N){
  T_rep_i <- rep(0, 500)
  for(j in 1:500){
    T_rep_i[j] <- sum((rep_values_34[[i]][j,1:HR.data$n[i]]-fitted_values_34[[i]][j,1:HR.data$n[i]])^2/
                        (sigma_wp[j,i]^2*HR.data$I_prepare[i,1:HR.data$n[i]]+
                           sigma_wm1[j,i]^2*HR.data$I_measure1[i,1:HR.data$n[i]]+
                           sigma_wt[j,i]^2*HR.data$I_task[i,1:HR.data$n[i]]+
                           sigma_wm2[j,i]^2*HR.data$I_measure2[i,1:HR.data$n[i]]+
  ...
  T_rep_34 <- append(T_rep_34, list(T_rep_i))
}

PPD34 <- rep(0, HR.data$N)
for(i in 1:HR.data$N){
  PPD34[i] <- mean(T_obs_34[[i]]<T_rep_34[[i]])
}
hist(PPD34)
summary(PPD34)

save(PPD34, file = "PPD34.Rdata")
save(fitted_values_34, file = "fitted_values_34.Rdata")
save(f_values_34, file = "f_values_34.Rdata")
save(rep_values_34, file = "rep_values_34")
save(T_rep_34, file = "T_rep_34.Rdata")
save(T_obs_34, file = "T_obs_34.Rdata")


## posterior predictive check for the outcome
fitted_outcome <- list()
T_rep_34_outcome <- rep(0, 500)
for(j in 1:500){
  ...
  T_rep_34_outcome[j] <- sum(fit_j)
}

mean(sum(HR.data$group) < T_rep_34_outcome)

## check the tail of the posterior predictive distribution
set.seed(886)
Tk_rep_34_outcome <- matrix(data = NA, nrow = 10, ncol = 500)

Tk_obs_34_outcome <- matrix(data = NA, nrow = 10, ncol = 500)
...
for(j in 1:500){
  ...
  for(k in 1:10){
    ...
    Tk_obs_34_outcome[k,j] <- sum(HR.data$group[which(decile==k)])
    Tk_rep_34_outcome[k,j] <- sum(fit_j[which(decile==k)])
  }
}

for(k in 1:10){
  print(mean(Tk_obs_34_outcome[k,] < Tk_rep_34_outcome[k,]))
}

save(fitted_outcome, file = "fitted_outcome.Rdata")
save(T_rep_34_outcome, file = "T_rep_34_outcome.Rdata")
save(Tk_obs_34_outcome, file = "Tk_obs_34_outcome.Rdata")
save(Tk_rep_34_outcome, file = "Tk_rep_34_outcome.Rdata")
