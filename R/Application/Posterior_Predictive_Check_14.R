...
comparison_group_ID <- unique(survey$idno[survey$group %in% c(1,4)])
...
HR.data <- joint_data_period_outcome(combined_data = combined_study_cut,
                                     n_subject = 61, nq = 2, survey = survey)
table(HR.data$group)
...
for(i in 1:HR.data$N){
  ...
  fitted_values_14 <- append(fitted_values_14, list(mu_i))
  f_values_14 <- append(f_values_14, list(fit_i))
}
...
for(i in 1:HR.data$N){
  ...
  for(j in 1:500){
    ...
    e_i <- rep(0, HR.data$n[i])
    e_i[1] <- AR1[j,i]*e_i0 + w[1]
    rep_i[j,1] <- f_values_14[[i]][j,1] + e_i[1]
    for(s in 2:HR.data$n[i]){
      e_i[s] <- AR1[j,i]*e_i[s-1] + w[s]
      rep_i[j,s] <- f_values_14[[i]][j,s] + e_i[s]
    }
  }
  rep_values_14 <- append(rep_values_14, list(rep_i))
}
...
for(i in 1:HR.data$N){
  T_obs_i <- rep(0, 500)
  for(j in 1:500){
    T_obs_i[j] <- sum((HR.data$HR[i,1:HR.data$n[i]]-fitted_values_14[[i]][j,1:HR.data$n[i]])^2/
  ...
  T_obs_14 <- append(T_obs_14, list(T_obs_i))
}
...
for(i in 1:HR.data$N){
  T_rep_i <- rep(0, 500)
  for(j in 1:500){
    T_rep_i[j] <- sum((rep_values_14[[i]][j,1:HR.data$n[i]]-fitted_values_14[[i]][j,1:HR.data$n[i]])^2/
                        (sigma_wp[j,i]^2*HR.data$I_prepare[i,1:HR.data$n[i]]+
                           sigma_wm1[j,i]^2*HR.data$I_measure1[i,1:HR.data$n[i]]+
                           sigma_wt[j,i]^2*HR.data$I_task[i,1:HR.data$n[i]]+
                           sigma_wm2[j,i]^2*HR.data$I_measure2[i,1:HR.data$n[i]]+
  ...
  T_rep_14 <- append(T_rep_14, list(T_rep_i))
}

PPD14 <- rep(0, HR.data$N)
for(i in 1:HR.data$N){
  PPD14[i] <- mean(T_obs_14[[i]]<T_rep_14[[i]])
}
hist(PPD14)
summary(PPD14)

save(PPD14, file = "PPD14.Rdata")
save(fitted_values_14, file = "fitted_values_14.Rdata")
save(f_values_14, file = "f_values_14.Rdata")
save(rep_values_14, file = "rep_values_14")
save(T_rep_14, file = "T_rep_14.Rdata")
save(T_obs_14, file = "T_obs_14.Rdata")


## posterior predictive check for the outcome
fitted_outcome <- list()
T_rep_14_outcome <- rep(0, 500)
for(j in 1:500){
  ...
  T_rep_14_outcome[j] <- sum(fit_j)
}

mean(sum(HR.data$group) < T_rep_14_outcome)

## check the tail of the posterior predictive distribution
set.seed(615)
Tk_rep_14_outcome <- matrix(data = NA, nrow = 10, ncol = 500)

Tk_obs_14_outcome <- matrix(data = NA, nrow = 10, ncol = 500)
...
for(j in 1:500){
  ...
  for(k in 1:10){
    ...
    Tk_obs_14_outcome[k,j] <- sum(HR.data$group[which(decile==k)])
    Tk_rep_14_outcome[k,j] <- sum(fit_j[which(decile==k)])
  }
}

for(k in 1:10){
  print(mean(Tk_obs_14_outcome[k,] < Tk_rep_14_outcome[k,]))
}

save(fitted_outcome, file = "fitted_outcome.Rdata")
save(T_rep_14_outcome, file = "T_rep_14_outcome.Rdata")
save(Tk_obs_14_outcome, file = "Tk_obs_14_outcome.Rdata")
save(Tk_rep_14_outcome, file = "Tk_rep_14_outcome.Rdata")
